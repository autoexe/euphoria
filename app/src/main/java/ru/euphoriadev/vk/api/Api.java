...
import ru.euphoriadev.vk.api.model.NewsfeedVideo;
...
//        }
    ...
    public ArrayList<Item> getNewsfeedVideo() throws IOException, JSONException, KException{
        String video = "video";
        String start_from = "";
        VKParams params = new VKParams("newsfeed.get");
        params.put("filters", video);
        params.put("return_banned", 1);
        params.put("count", 6);
        params.put("start_from", start_from);
        JSONObject root = sendRequest(params);
        JSONObject response = root.optJSONObject("response");
        ArrayList<Item> newsfeedVideos = new ArrayList<Item>();
        //if (response == null)
            //return newsfeedVideos;
        //JSONArray array = response.optJSONArray("items");
       // if (array != null){
           // for (int i = 0; i < array.length(); i++) {
               // JSONObject o = (JSONObject) array.get(i);
               // NewsfeedVideo svideo = NewsfeedVideo.parse(o);
                //newsfeedVideos.add(svideo);
           // }
        //}
        NewsfeedVideo svideo = NewsfeedVideo.parse(response);
