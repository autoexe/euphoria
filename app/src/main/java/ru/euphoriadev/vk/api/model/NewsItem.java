...
public class NewsItem {
    ...
    public Geo geo;
    public ArrayList<VKPhoto> photos = new ArrayList<VKPhoto>(); //except wall
    public ArrayList<VKPhoto> photo_tags = new ArrayList<VKPhoto>();//except wall
    public ArrayList<VKNote> notes;//except wall
    ...
    public static NewsItem parse(JSONObject jitem, boolean is_comments) throws JSONException {
        ...
        if (copy_history_json != null) {
            newsitem.copy_history = new ArrayList<VKWallMessage>();
            for (int i = 0; i < copy_history_json.length(); ++i) {
                try {
                    newsitem.copy_history.add(VKWallMessage.parse(copy_history_json.getJSONObject(i)));
                } catch (Throwable th) {
                    th.printStackTrace();
        ...
        newsitem.attachments = VKAttachment.parseAttachments(attachments, newsitem.source_id, newsitem.copy_owner_id, geo_json);
        ...
        if (newsitem.type.equals("photo") && is_comments) {
            newsitem.photos = new ArrayList<VKPhoto>();
            VKPhoto photo = VKPhoto.parse(jitem);
            newsitem.photos.add(photo);
        }
