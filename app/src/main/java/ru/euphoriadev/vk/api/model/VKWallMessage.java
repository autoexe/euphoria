...
public class VKWallMessage implements Serializable {
    ...
    public static VKWallMessage parse(JSONObject o) {
        VKWallMessage wm = new VKWallMessage();
        wm.id = o.optLong("id");
        wm.from_id = o.optLong("from_id");
        if (o.has("reply_post_id"))
            wm.reply_post_id = o.optLong("reply_post_id");
        //     if(o.has("to_id"))
        //         wm.to_id = o.getLong("to_id");
        //     else
        //in copy_history owner_id is used
        //         wm.to_id = o.getLong("owner_id");
        wm.owner_id = o.optLong("owner_id");
        wm.date = o.optLong("date");
        wm.post_type = getPostType(o);
        wm.text = Api.unescape(o.optString("text"));
        if (o.has("likes")) {
            JSONObject jlikes = o.optJSONObject(NewsJTags.LIKES);
        ...
        if (o.has(NewsJTags.REPOSTS)) {
            JSONObject jlikes = o.optJSONObject(NewsJTags.REPOSTS);
    ...
    public static VKWallMessage parseForNotifications(JSONObject o) throws JSONException {
        VKWallMessage wm = new VKWallMessage();
        wm.id = o.getLong("id");
        ...
        //likes is there but I don't parseArray it because I don't need it
        //if (o.has("likes")){
